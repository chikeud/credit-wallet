...
export async function seed(knex: Knex): Promise<void> {
    // Start a transaction for identities insertion
    const trx = await knex.transaction();

    try {
        await trx('identities').delete();

        // Insert identities with JSON columns as JSON strings
        ...
        // Fetch the ids of the inserted identities after commit
        const identities = await trx('identities').select('id');

        // Commit the transaction after identities are successfully inserted
        await trx.commit();
